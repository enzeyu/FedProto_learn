...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        # '''x:(4,3,32,32)->(4,64,16,16)->(4,64,16,16)->(4,64,8,8)->(4,64,8,8)->(4,128,4,4)->(4,256,2,2)'''
        # '''x1:(4,512,1,1)'''
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)
        ...
        #  对输入x1全局平均池化，将每个通道上的所有元素取平均值
        x = self.avgpool(x1)
        #  将经过全局平均池化的结果 x 进行形状变换，将其变为一个一维向量。这通常是为了将特征图展平，以便连接到全连接层。
        x = x.view(x.size(0), -1)
        # 将x输入到全连接层
        x = self.fc(x)

        # 对x进行log softmax操作。这通常在多分类问题模型的输出层中使用，以获得每个类别的对数概率
